...
mcpServer.tool(
  ...
  async (): Promise<any> => {
    ...
    const requestId = uuidv4();
    const payload = { type: "REQUEST_SCHEMA_MARKUP", requestId, timestamp: Date.now() } as const;
...
mcpServer.tool(
  ...
  async (): Promise<any> => {
    ...
    const requestId = uuidv4();
    const payload = { type: "REQUEST_GTM_CONTAINER_IDS", requestId, timestamp: Date.now() } as const;
