// metaPixelHits.ts - Meta Pixel hits MCP tool
    ...
    mcpServer.tool("getMetaPixelHits", "Get all Meta Pixel (Facebook Pixel) hits recorded from the human's attached browser tab via the connected extension. Recording is automatic and resets on page navigation.", {}, async () => {
        ...
        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                cleanup();
                logWarn(`Meta Pixel hits request ${requestId} timed out`);
                resolve({
                    content: [
                        {
                            type: "text",
                            text: "Timeout waiting for Meta Pixel hits from extension. The extension may be busy or disconnected.",
