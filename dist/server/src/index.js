...
function wsSend(socket, data, retries = 3) {
...
mcpServer.tool("getDataLayer", "Capture and return the full contents of window.dataLayer (as JSON) from the active browser tab, allowing inspection of all GTM events.", {}, async () => {
    ...
    const payload = { type: "REQUEST_DATALAYER", requestId, timestamp: Date.now() };
...
mcpServer.tool("getMetaPixelHits", "Get all Meta Pixel (Facebook Pixel) hits recorded from the current page. Recording is automatic and resets on page navigation.", {}, async () => {
    ...
    const requestId = uuidv4();
    const payload = { type: "REQUEST_META_PIXEL_HITS", requestId, timestamp: Date.now() };
    if (!wsSend(socket, payload)) {
        return {
            content: [
                {
...
mcpServer.tool("getSchemaMarkup", "Extract and return all schema markup (JSON-LD and microdata) found on the current page, including structured data for SEO and rich snippets.", {}, async () => {
    ...
    if (!connectionState.isHealthy) {
        logWarn("Attempting getSchemaMarkup on potentially unhealthy connection");
    }
    const requestId = uuidv4();
    const payload = { type: "REQUEST_SCHEMA_MARKUP", requestId, timestamp: Date.now() };
    if (!wsSend(socket, payload)) {
        return {
            content: [
                {
    ...
    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            cleanup();
            logWarn(`Schema markup request ${requestId} timed out after ${CONNECTION_TIMEOUT}ms`);
            resolve({
                content: [
                    {
                        type: "text",
                        text: `Timeout waiting for schema markup from extension (${CONNECTION_TIMEOUT}ms). The extension may be busy or disconnected.`,
                        _meta: { isError: true, requestId, connectionState: "timeout" },
                    },
                ],
                isError: true,
        ...
        function handleMessage(data) {
            try {
                ...
                if (msg.type === "SCHEMA_MARKUP_RESPONSE" && msg.requestId === requestId) {
                    ...
                    connectionState.lastActivity = Date.now();
                    if (msg.payload?.error) {
                        logWarn(`Schema markup request ${requestId} returned error:`, msg.payload.error);
                        resolve({
                            content: [
                                {
                                    type: "text",
                        ...
                        logInfo(`Schema markup request ${requestId} completed successfully`);
                        resolve({
                            content: [
                                {
                                    type: "text",
                                    ...
                                    _meta: {
                                        requestId,
                                        jsonLdCount: msg.payload?.summary?.jsonLdCount || 0,
                                        microdataCount: msg.payload?.summary?.microdataCount || 0,
                                        processingTime: msg.payload?.processingTime || 0
                                    }
                                },
                            ],
                        });
        ...
        function handleClose() {
            cleanup();
            logWarn(`WebSocket closed while waiting for schema markup request ${requestId}`);
            resolve({
                content: [
                    {
                        type: "text",
        ...
        function handleError(error) {
            cleanup();
            logError(`WebSocket error while waiting for schema markup request ${requestId}:`, error);
            resolve({
                content: [
                    {
                        type: "text",
...
mcpServer.tool("getGTMContainerIds", "Extract and return all (normally just one) Google Tag Manager container IDs installed on the current page from the window.google_tag_manager object.", {}, async () => {
    ...
    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            cleanup();
            logWarn(`GTM container IDs request ${requestId} timed out after ${CONNECTION_TIMEOUT}ms`);
            resolve({
                content: [
                    {
                        type: "text",
                        text: `Timeout waiting for GTM container IDs from extension (${CONNECTION_TIMEOUT}ms). The extension may be busy or disconnected.`,
                        _meta: { isError: true, requestId, connectionState: "timeout" },
                    },
                ],
                isError: true,
